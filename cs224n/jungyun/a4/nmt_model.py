...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings_source(source_padded)  # 패딩된 문장 임베딩

        X = pack_padded_sequence(X, source_lengths) #PackedSequence Object
        # 인코딩
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        # 인코딩 후
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2)  # 이를 다시 원래 형태의 (배치크기, 문장의 최대 길이, 히든크기) 로 바꾸려면 pad_packed_sequence 를 사용
                                                                            # permute - 모든 dim에 대해서 transpose 0->1, 1->0, 2->2으로 차원변경
        # 디코더 init
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # att_projection 레이어에 인코 히든 스테이트를 연산 --> enc_hiddens_proj


        Y = self.model_embeddings.target(target_padded)  # 타켓 모델(타겟 패딩 문장 입력으로 받는) 이용해서 타겟 문장 임베딩
        ...
        combined_outputs = torch.stack(
            combined_outputs, dim=0)  # 아웃풋을 싱글 텐서 타입으로 모양 바꾸기
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Ybar_t에 디코더 연산해서 전 히든/셀 디코더 스테이트 결과를 같이 가져올 수 있도록!
        dec_prev_h, dec_prev_c = dec_state # 이전 히든 , 셀 값
        dec_state = self.decoder(Ybar_t, (dec_prev_h, dec_prev_c)) # 현 디코더 스테이트


        dec_hidden, dec_cell = dec_state  # 히든과 셀 값으로 분리

        # 어텐션 스코어 e_t 계산 ( 몰라!!!!!!)
        dec_hidden_unsqueezed = dec_hidden.unsqueeze(dim=2)
        # enc_hiddens_proj (b, src_len, h)
        e_t = enc_hiddens_proj.bmm(dec_hidden_unsqueezed)  # bmm - batch matrix-matrix product (bmm은 broadcast X)
        e_t = e_t.squeeze(dim=2)  # remove 2 dims
        ...
        # e_t 소프트맥스
        alpha_t = torch.nn.functional.softmax(e_t, dim=1)  # 어텐션 디스트리뷰션
        ...
        U_t = torch.cat([a_t, dec_hidden], dim=1)  # (b, 3h) Concatentate dec_hidden & a_t

        # combined output projection layer to U_t
        ...
        combined_output = O_t
        return dec_state, combined_output, e_t # e_t (어텐션 스코어)
