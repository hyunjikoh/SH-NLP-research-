    ...
    for sent in sents:
        ...
        for word in sent:
            if len(word) < max_word_length:
                padding = [char_pad_token] * (max_word_length - len(word)) ## 사이즈에 못 미치는 것들 0 추가
                word.extend(padding)
            elif len(word) > max_word_length:
                word = word[0:max_word_length] ## 사이즈 이상 되는 것들 자르기
            sent_padded.append(word)
        if len(sent_padded) < max_sent_length:
            new_sent = [char_pad_token] * max_word_length
            padding = [new_sent] * (max_sent_length - len(sent_padded))
            sent_padded += padding
