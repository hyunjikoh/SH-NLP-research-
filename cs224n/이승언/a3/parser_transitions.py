...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        mini_parse = list(unfinished_parses[:batch_size])
        next_transition = model.predict(mini_parse)
        for i, parse in enumerate(mini_parse):
            parse.parse_step(next_transition[i])
