...
def naiveSoftmaxLossAndGradient(
    ...
    y = np.zeros(len(outsideVectors))  # (V,)
    y[outsideWordIdx] = 1.0  # (N,)

    y_hat = softmax(np.matmul(centerWordVec, np.transpose(outsideVectors)))
    loss = - np.log(y_hat[outsideWordIdx])  # (1,)

    gradCenterVec = np.matmul(outsideVectors.T, y_hat - y)  # (V,)
    gradOutsideVecs = np.matmul(np.asmatrix(y_hat - y).T, np.asmatrix(centerWordVec))  # (N,V)
