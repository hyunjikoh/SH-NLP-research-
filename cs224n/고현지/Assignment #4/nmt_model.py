...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # bidirectional 이라서 hidden_size * 2
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Source Sentence가 배치크기로 들어
        X = self.model_embeddings.source(source_padded)  # (src_len, b, e)
        # 각 타임 스탭 (time step) 별 병렬 계산을 진행할 때 쓸모없는 pad 계산을 최대한 줄이려고 할 때 사용
        X = pack_padded_sequence(X, source_lengths)

        # enc_hiddens => hi^enc (src_len,b,2*h)
        # self.encoder = nn.LSTM(embed_size, hidden_size, bias=True, bidirectional=True)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)  # last_hidden/last_cell = (2, b, h)
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0]  # (src_len, b, 2*h)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, 2*h)

        # last_hidden[0] : forward -> 모양   /    last_hidden[1] : backward <- 모양
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # squeeze => 차원 줄이기  a => torch.Size([1, 128, 128])  a = torch.squeeze(a) => torch.Size([128, 128])
        # unsqueeze => 차원 늘기  a => torch.Size([128, 128]) a = torch.unsqueeze(a, 0) => torch.Size([1, 128, 128])
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        # bmm은 matrix multiplication으로, [n, m] x [m,p] = [n,p] 를 구현한다.
