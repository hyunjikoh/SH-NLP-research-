...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        # char_list를 각각 integer로 변환하여 char2id에 dictionary 형태로 넣
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
            S2 = []
            for w in s:
                X3 = [self.start_of_word] # '{'
                for c in w:
                    # char2id는 A부터 특수문자까지 integer로 변환되어있는 dictionary
                    X3.append(self.char2id.get(c, self.char_unk)) # character(c)를 dict에 넣어 int형으 가져오되, 없으면 '<unk>'

                X3.append(self.end_of_word) # '}'
                S2.append(X3)
            S1.append(S2)
