...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    #p = softmax(centerWordVec @ outsidVectors.T)

    print('outsideVectors.shape : ',outsideVectors.shape) # (5,3)
    print('centerWordVec.shape : ',centerWordVec.shape) # (3,)
    # 자료의 (2) 식을 구현함
    J = softmax(np.matmul(outsideVectors, np.transpose(centerWordVec))) # (1)
    J_o = J[outsideWordIdx]
    loss = -np.log(J_o) # (2)
    ...
    gradCenterVec = np.matmul(J - y, outsideVectors)    # (3,)
    gradOutsideVecs = np.outer(J - y, centerWordVec)    # (5,3)
    ### END YOUR CODE
...
def getNegativeSamples(outsideWordIdx, dataset, K):
    ...
    print('outsideWordIdx : ', outsideWordIdx, ' || negSampleWordIndices : ', negSampleWordIndices)
