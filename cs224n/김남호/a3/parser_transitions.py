...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        next_transitions = model.predict(partial_parses=mini_batch)
        #print('next_transition : ', next_transitions)
        for each_pp, each_trans in zip(mini_batch, next_transitions):
            each_pp.parse_step(each_trans)
            #print('pp,stack : ', each_pp.stack, 'pp.buffer : ', each_pp.buffer)
            if len(each_pp.stack) == 1 and not each_pp.buffer:
                unfinished_parses.remove(each_pp)
                #print('unfinished_parsed removed')
