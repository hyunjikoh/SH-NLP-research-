...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id[c] for c in f'{"{"}{w}{"}"}'] for w in s] for s in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.tensor(padded_sentences, dtype=torch.long, device=device) # (batch_size, max_sentence_length, max_word_length)
