...
def minibatch_parse(sentences, model, batch_size):
    ...
    undefined_parses = partial_parses
    while undefined_parses:
        # batch_size만큼 parser를 가져와서 model로 다음 transition을 에측
        mini_batch = undefined_parses[:batch_size]
        transitions = model.predict(mini_batch)
        # 각 예측된 값을 이용해 parser
        for transition, parser in zip(transitions, mini_batch):
            parser.parse_step(transition)
        # 완료된 parser는 제거
        undefined_parses = [
            x for x in undefined_parses 
                if len(x.stack) > 1 or len(x.buffer) > 0]
    # 최종적으로 dependencies만 가져온다
