...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        def word_emb(word):
            """ word가 들어오면 character id 리스트로 변경해주는 함수. 시작과 끝에 start, end token을 붙여준다.
            """
            return [self.start_of_word] + [self.char2id[c] for c in word] + [self.end_of_word]

        word_ids = [[word_emb(w) for w in s] for s in sents]
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # (b, sent_len, word_len) -> (sent_len, b, word_len)
